...
class Kelompok @Inject()(cc: ControllerComponents) extends AbstractController(cc) {
  ...
  def hitung_segitiga(alas:String,tinggi:String) = Action { 
    ...
    val karson = new PrintWriter(writer)

    karson.println("Segitiga:alas = "+alas+" tinggi = "+tinggi+":"+hasil)
    karson.close()
     Ok(hasil.toString())
  }
  ...
  def hitung_pp(p:String,l:String) = Action { 
    ...
    karson.println("Persegi Panjang:panjang = "+p+" lebar = "+l+":"+hasil)
    karson.close()
     Ok(hasil.toString())
  }
  ...
  def hitung_lingkaran(jari:String) = Action { 
    ...
    karson.println("Lingkaran:jari-jari = "+jari+":"+hasil)
    karson.close()
     Ok(hasil.toString())
  }
